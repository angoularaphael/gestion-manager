...
/** Valeurs affichées pour le suivi WhatsApp offre été (override optionnel sur la base). */
export function getOffreEteWhatsAppDisplayStats() {
  const sentRaw = process.env.OFFRE_ETE_WHATSAPP_SENT_OVERRIDE;
  const readRaw = process.env.OFFRE_ETE_WHATSAPP_READ_OVERRIDE;
  const whatsappSentOverride =
    sentRaw != null && sentRaw !== '' && Number.isFinite(Number(sentRaw)) ? Number(sentRaw) : null;
  const whatsappReadOverride =
    readRaw != null && readRaw !== '' && Number.isFinite(Number(readRaw)) ? Number(readRaw) : null;
  return { whatsappSentOverride, whatsappReadOverride };
}
