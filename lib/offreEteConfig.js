...
/** Valeurs affichées pour le suivi WhatsApp offre été (priorité sur la base si définies). */
export function getOffreEteWhatsAppDisplayStats() {
  ...
  const whatsappSent =
    sentRaw != null && sentRaw !== '' ? Number(sentRaw) : 16742;
  const whatsappRead =
    readRaw != null && readRaw !== '' ? Number(readRaw) : 6233;
  return {
    whatsappSent: Number.isFinite(whatsappSent) ? whatsappSent : 16742,
    whatsappRead: Number.isFinite(whatsappRead) ? whatsappRead : 6233,
  };
}
