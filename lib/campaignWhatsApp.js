  ...
  if (testOnly) {
    ...
    const data = await botFetch('/api/send-to-clients', {
      ...
      body: {
        ...
        channels: ['whatsapp'],
        campaign_kind: kind,
        offre_ete_whatsapp: false,
        message: 'campaign',
        max_whatsapp_sends: 1,
      },
    });
    const waSent = data?.whatsapp?.sent > 0 || data?.whatsapp?.queued > 0;
    return {
      testOnly: true,
      ...
      try {
        const data = await botFetch('/api/send-to-clients', {
          ...
          body: {
            ...
            offre_ete_whatsapp: false,
            message: 'campaign',
            max_whatsapp_sends: MESSAGES_PER_BOT_PER_WAVE,
          },
          timeoutMs: 8000,
        });
        const assigned = Math.min(
          MESSAGES_PER_BOT_PER_WAVE,
          data?.whatsapp?.queued ?? batch.filter((c) => c.telephone).length
        );
