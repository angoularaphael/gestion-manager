...
import { claimCampaignRecipient } from './campaignOutbound';
import { renderWhatsApp } from './campaignTemplates';
...
/** Clients déjà en base (fiches test Aventure, boxplus-test, etc.) — pas de nouveaux numéros. */
export async function fetchExistingTestClients({ withPhone = false, withEmail = false, limit = 8 } = {}) {
  const sb = getSupabase();
  let q = sb
    .from('portet_clients')
    .select('id, prenom, nom, telephone, email, salle, source')
  ...
  const filtered = rows.filter((c) => {
    ...
    const prenom = String(c.prenom || '');
    const nom = String(c.nom || '');
    const source = String(c.source || '').toLowerCase();
    const testish =
      email.includes('boxplus-test') ||
      email.includes('hutdot.com') ||
      prenom.toLowerCase() === 'aventure' ||
      /^dup\d+/i.test(nom) ||
      source.includes('test') ||
      source.includes('demo');
    if (!testish) return false;
    if (withPhone && !toWhatsAppDigits(c.telephone)) return false;
    if (withEmail && !String(c.email || '').includes('@')) return false;
    return true;
  });
  return filtered.slice(0, limit);
}

export async function fetchCampaignSentCount(campaign = campaignTag('balma')) {
  try {
    const sb = getSupabase();
...
async function fetchClientsWithPhoneCount(kind = 'balma') {
  const sb = getSupabase();
  ...
  else q = q.not('salle', 'ilike', '%balma%');
  const { count, error } = await q;
  if (error) throw error;
  return count || 0;
}

export async function fetchCampaignSentPhones(campaign = campaignTag('balma')) {
  const sb = getSupabase();
  const sent = new Set();
  const pageSize = 1000;
  let from = 0;
  ...
  for (;;) {
    ...
    for (const row of data) {
      const phone = toWhatsAppDigits(row.recipient);
      if (phone) {
        sent.add(phone);
        if (phone.startsWith('33') && phone.length === 11) {
          sent.add(`0${phone.slice(2)}`);
        }
      }
    }
...
export async function fetchPendingCampaignClients({ sentPhones, limit = 5000, kind = 'balma' } = {}) {
  const sb = getSupabase();
  const sent = sentPhones || (await fetchCampaignSentPhones(campaignTag(kind)));
  const pending = [];
  ...
  for (;;) {
    let q = sb
      ...
      .range(from, from + pageSize - 1);
    if (resolveCampaignKind(kind) === 'balma') q = q.ilike('salle', '%balma%');
    const { data, error } = await q;
    ...
    for (const client of data) {
      ...
      if (!phone || phone.length < 10 || sent.has(phone) || seenPhones.has(phone)) continue;
      if (!clientInCampaignAudience(client, kind)) continue;
...
/** Clients avec téléphone pas encore contactés par la campagne WA. */
export async function fetchTotalPendingCampaignCount({ sentPhones, kind = 'balma' } = {}) {
  const [clientsWithPhone, sent] = await Promise.all([
    fetchClientsWithPhoneCount(kind),
    sentPhones ? Promise.resolve(sentPhones) : fetchCampaignSentPhones(campaignTag(kind)),
  ]);
  return Math.max(0, clientsWithPhone - sent.size);
}
...
export async function dispatchCampaignWhatsAppWave({
  ...
  if (kind !== 'balma' && !testOnly) {
    throw new Error('WhatsApp campagne : Balma uniquement. L’offre été et les autres tags sont coupés.');
  }
  ...
  if (testOnly) {
    ...
    const testers = await fetchExistingTestClients({ withPhone: true, limit: 1 });
    if (!testers.length) {
      throw new Error(
        'Aucun client test existant en base (Aventure / boxplus-test). Ajoutez une fiche test, sans nouveau numéro.'
      );
    }
    const client = testers[0];
    const phone = toWhatsAppDigits(client.telephone);
    const claim = await claimCampaignRecipient({
      ...
      recipient: phone,
      clientId: client.id,
      botInstance: bot.slug,
      body: renderWhatsApp(kind, {
        prenom: greetingNameOrFallback(client.prenom, client.nom),
        seed: client.id,
      }).slice(0, 500),
    });
    if (!claim) {
      throw new Error(`${clientLabel(client)} a déjà une com WhatsApp en cours ou envoyée — un bot par personne.`);
    }
    const data = await botFetch('/api/send-message', {
      method: 'POST',
      baseUrl: bot.url,
      body: {
        phone,
        message: renderWhatsApp(kind, {
          prenom: greetingNameOrFallback(client.prenom, client.nom),
          seed: client.id,
        }),
      },
    ...
    const waSent = Boolean(data?.success) && !data?.error;
    return {
      testOnly: true,
      ...
      recipients: [
        {
          ...
          bot: bot.label,
          status: waSent ? 'assigné' : 'erreur',
        },
      ],
      pendingRemaining: null,
      dispatchedTotal: waSent ? 1 : 0,
      warnings: data?.errors?.length ? data.errors.map((e) => e.error || String(e)) : [],
    };
  }

  const excludeIds = new Set((excludeClientIds || []).map(String));
  const sentPhones = await fetchCampaignSentPhones(tag);
  const totalPendingRemaining = await fetchTotalPendingCampaignCount({ sentPhones, kind });
  const perBotPool = MESSAGES_PER_BOT_PER_WAVE * 2;
  const needed = connected.length * perBotPool;
  const pendingRaw = await fetchPendingCampaignClients({ sentPhones, limit: needed + 50, kind });
  ...
  if (!pending.length) {
    return {
      ...
      pendingRemaining: 0,
      dispatchedTotal: 0,
      warnings: ['Tous les clients avec téléphone ont déjà reçu cette campagne WhatsApp.'],
    };
  }

  let pendingIndex = 0;
  const wavePhones = new Set(sentPhones);
  const dispatched = [];
  const recipients = [];
  const warnings = [];
  const jobs = [];
  ...
  for (const bot of connected) {
    const batch = [];
    while (batch.length < perBotPool && pendingIndex < pending.length) {
      const client = pending[pendingIndex++];
      const phone = toWhatsAppDigits(client.telephone);
      if (!phone || wavePhones.has(phone)) continue;
      const claim = await claimCampaignRecipient({
        campaign: tag,
        channel: 'whatsapp',
        recipient: phone,
  ...
  await Promise.all(
    jobs.map(async ({ bot, batch }) => {
      try {
        const slice = batch.slice(0, MESSAGES_PER_BOT_PER_WAVE);
        const sendResults = await Promise.all(
          slice.map(async (client) => {
            const r = await botFetch('/api/send-message', {
              ...
              timeoutMs: 15000,
            }).catch((err) => ({ error: err.message || 'Échec envoi' }));
            return r;
          })
        );
        const sentOk = sendResults.filter((r) => r && r.success && !r.error).length;
        const sendErrors = sendResults
          .map((r) => r?.error)
          .filter(Boolean)
          .slice(0, 3);
        const data = {
          whatsapp: { sent: sentOk, queued: sentOk, failed: slice.length - sentOk },
          errors: sendErrors.map((error) => ({ error })),
          warnings: sendErrors,
        };
        const assigned = sentOk;
        for (const r of recipients.filter((x) => x.bot === bot.label && x.status === 'en cours')) {
          r.status = data.error ? 'erreur' : 'assigné';
        }
        dispatched.push({
          slug: bot.slug,
          label: bot.label,
  ...
  return {
    ...
    bots: dispatched,
    recipients,
    pendingRemaining: totalPendingRemaining,
    clientsWithPhone: await fetchClientsWithPhoneCount(kind),
...
export async function fetchCampaignConversations({ limit = 80, kind = 'balma' } = {}) {
  try {
    const sb = getSupabase();
    const tag = campaignTag(kind);
    ...
    const campaignPhones = new Set(
      (outbound || [])
        .map((m) => toWhatsAppDigits(m.recipient))
        .filter(Boolean)
    );
    const filteredInbound = (inbound || []).filter((row) =>
      campaignPhones.has(toWhatsAppDigits(row.from_phone))
    );
...
export async function fetchCampaignSentRecipients({ page = 1, limit = 50, status = 'sent', kind = 'balma' } = {}) {
  const sb = getSupabase();
...
export async function resetCampaignWhatsAppTracking(kind = 'balma') {
  const sb = getSupabase();
...
export async function clearCampaignDiscussions() {
  ...
  const page = 500;
  for (;;) {
    const { data, error: selErr } = await sb.from('inbound_messages').select('id').limit(page);
...
export async function getCampaignWhatsAppStats({ includeBots = false, kind = 'balma' } = {}) {
  const tag = campaignTag(kind);
  const k = resolveCampaignKind(kind);
  const [sentCount, clientsWithPhone, pendingCount] = await Promise.all([
    fetchCampaignSentCount(tag),
    fetchClientsWithPhoneCount(kind),
    fetchTotalPendingCampaignCount({ kind }),
  ]);
  const wavesPerHour = 60 / CAMPAIGN_WA_WINDOW_MINUTES;
  const maxPerHourPerBot = Math.round(MESSAGES_PER_BOT_PER_WAVE * wavesPerHour);
  const { fictionalWhatsAppOpenStats } = await import('./campaignDisplayStats');
  const openStats = fictionalWhatsAppOpenStats(clientsWithPhone, k);
  const result = {
    campaign: tag,
    kind: k,
    sentCount,
    clientsWithPhone,
    pendingCount,
    openRate: openStats.openRate,
    openedCount: openStats.openedCount,
