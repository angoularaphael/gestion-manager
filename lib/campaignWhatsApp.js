...
export async function dispatchCampaignWhatsAppWave({ testOnly = false } = {}) {
  ...
  const pending = await fetchPendingCampaignClients({ sentPhones });
  if (!pending.length) {
    return {
      bots: [],
      pendingRemaining: 0,
...
export async function getCampaignWhatsAppStats() {
  const sentPhones = await fetchCampaignSentPhones();
  const pending = await fetchPendingCampaignClients({ sentPhones, limit: 100000 });
  const bots = await probeCampaignBots();
  return {
    ...
    maxPerHour: MESSAGES_PER_BOT_PER_HOUR * bots.filter((b) => b.connected).length,
    bots,
  };
}
