...
export async function fetchTotalPendingCampaignCount({ sentPhones } = {}) {
  const [clientsWithPhone, sent] = await Promise.all([
    fetchClientsWithPhoneCount(),
    sentPhones ? Promise.resolve(sentPhones) : fetchCampaignSentPhones(),
  ]);
...
export async function dispatchCampaignWhatsAppWave({
  ...
  if (testOnly) {
    ...
    const waFailed = data?.whatsapp?.failed > 0;
    const status = waSent ? 'envoyé' : waFailed ? 'erreur' : 'ignoré';
    const warnings = [];
    if (!waSent && !waFailed) {
      warnings.push(
        `Test non envoyé (${data?.whatsapp?.skipped ? 'numéro ignoré' : 'vérifiez le bot'}) — le mode test ne compte pas dans la campagne.`
      );
    }
    if (data?.errors?.length) {
      warnings.push(...data.errors.map((e) => e.error || String(e)));
    }
    return {
      testOnly: true,
  ...
  const totalPendingRemaining = await fetchTotalPendingCampaignCount({ sentPhones });
    ...
    try {
      const data = await botFetch('/api/send-to-clients', {
        ...
        body: {
          ...
          channels: ['whatsapp'],
          campaign_kind: process.env.CAMPAIGN_KIND || 'balma',
...
export async function getCampaignWhatsAppStats({ includeBots = false } = {}) {
  const [sentCount, clientsWithPhone, pendingCount] = await Promise.all([
    ...
    fetchClientsWithPhoneCount(),
    fetchTotalPendingCampaignCount(),
  ]);
