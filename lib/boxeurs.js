...
export async function fetchTestBoxeurFromDb() {
  const sb = getSupabase();
...
export async function resolveBoxeursForSend({ boxeur_ids: boxeurIds, test_only: testOnly, broadcast }) {
  ...
  return resolveContactsForSend({
    ...
    testOnly,
    broadcast,
    fetchTestRow: fetchTestBoxeurFromDb,
  });
}
