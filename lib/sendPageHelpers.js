export function mergeSendResults(target, source) {
  const countKey =
    source.managers != null
      ? 'managers'
      : source.promoteurs != null
        ? 'promoteurs'
        : source.clients != null
          ? 'clients'
          : source.boxeurs != null
...
export async function runDualChannelSend({
  ...
  if (channels.includes('whatsapp')) {
    await tryChannel('whatsapp', async () => {
      if (payload.offre_ete_whatsapp) {
        const waRes = await fetch('/api/campaign/whatsapp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            action: 'dispatch',
            test_only: Boolean(payload.test_only),
          }),
        });
        const waData = await parseApiJson(waRes);
        if (!waRes.ok) throw new Error(waData.error || 'Erreur campagne WhatsApp');
        if (payload.test_only) {
          return waData.bots?.[0] || waData;
        }
        const queued = waData.dispatchedTotal || 0;
        data.whatsapp.queued = queued;
        if (waData.warnings?.length) data.warnings.push(...waData.warnings);
        data.warnings.push(
          `Campagne 3 bots : ${queued} client(s) assigné(s) sur ${waData.connectedBots || 0} bot(s) — ` +
            `${(waData.pendingRemaining ?? 0).toLocaleString('fr-FR')} restant(s) avec téléphone (sur ${(waData.clientsWithPhone ?? 0).toLocaleString('fr-FR')} au total). ` +
            'Voir les discussions sur /admin/campagne-whatsapp.'
        );
        return { clients: queued, whatsapp: { sent: 0, queued, failed: 0, skipped: 0 } };
      }
