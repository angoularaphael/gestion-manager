...
export async function getCampaignSettings() {
  ...
  const row = data || defaultSettings();
  return {
    ...row,
    campaign: resolveCampaignKind(row.campaign),
  };
}
...
export async function updateCampaignSettings(patch) {
  ...
  if (error) {
    if (error.message && /column .*campaign/i.test(error.message) && 'campaign' in payload) {
      const { campaign: _c, ...rest } = payload;
      return updateCampaignSettings(rest);
    }
    throw error;
  }
  return {
    ...data,
    campaign: resolveCampaignKind(data?.campaign),
  };
}

export async function setCampaignActive(active, kind) {
  return updateCampaignSettings({
    active: Boolean(active),
    paused_at: active ? null : new Date().toISOString(),
    ...(kind ? { campaign: resolveCampaignKind(kind) } : {}),
  });
}
