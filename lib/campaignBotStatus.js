...
function statusFromPayload(data, { includeQr = false } = {}) {
  const hasQr = Boolean(data?.qr) && !data?.connected;
  const result = {
    configured: true,
    reachable: true,
    ...
    connectedWhatsApp: data?.connectedWhatsApp || null,
    hasQr,
    qrError: data?.qrError || null,
    pairingCode: data?.pairingCode || null,
    error: data?.qrError || data?.error || null,
  };
  if (includeQr && hasQr) result.qr = data.qr;
  return result;
}

export async function fetchCampaignBotStatus(botUrl, { includeQr = false } = {}) {
  ...
  try {
    ...
    return statusFromPayload(data, { includeQr });
  } catch (statusErr) {
