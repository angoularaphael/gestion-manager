...
import { buildOffreEteEmailBodyHtml } from './offreEteEmailHtml';
...
import { EMAIL_CID_LOGO, emailCidSrc, getEmailInlineAttachments } from './emailInlineImages';
...
export function buildListUnsubscribeHeaders(unsubscribeUrl) {
  ...
  return {
    ...
    'List-ID': '<clients.boxingcenter.fr>',
  };
}
...
export function buildMarketingEmailParts({
  ...
  const logoSrc = emailCidSrc(EMAIL_CID_LOGO);
  let inlinedAttachments = [];

  try {
    inlinedAttachments = getEmailInlineAttachments();
  } catch (e) {
    console.warn('[email] images inline indisponibles:', e.message);
  }
  ...
  const html = buildEmailHtml({
    ...
    contentHtml: buildOffreEteEmailBodyHtml(body),
    logoSrc,
  });
  ...
  const text = [
    body,
    '',
    `Commander l'offre : ${OFFRE_ETE_SHOP_URL}`,
    textFooter,
  ].join('\n');

  return { html, text, headers, unsubscribeUrl, inlinedAttachments };
}
