...
function resendDailyCap() {
  ...
  const n = parseInt(process.env.RESEND_DAILY_LIMIT || '100', 10);
  return Number.isFinite(n) && n > 0 ? n : 100;
}

export function getHourlyLimit() {
  const resendCap = resendDailyCap();
  if (resendCap) return resendCap;
  const n = parseInt(process.env.MAILJET_HOURLY_LIMIT || '200', 10);
