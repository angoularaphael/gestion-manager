...
function resendDailyCap() {
  const provider = (process.env.EMAIL_PROVIDER || 'auto').trim().toLowerCase();
  const usingResend =
    provider === 'resend' || (provider === 'auto' && Boolean(process.env.RESEND_API_KEY));
  if (!usingResend) return null;
  const n = parseInt(process.env.RESEND_DAILY_LIMIT || '1600', 10);
  return Number.isFinite(n) && n > 0 ? n : 100;
}

export function getHourlyLimit() {
  const hourly = parseInt(process.env.RESEND_HOURLY_LIMIT || process.env.MAILJET_HOURLY_LIMIT || '80', 10);
  const n = Number.isFinite(hourly) && hourly > 0 ? hourly : 80;
  const daily = resendDailyCap();
  if (daily) return Math.min(n, daily);
  return n;
}

export function getWaveLimit() {
  const hourly = getHourlyLimit();
  const n = parseInt(process.env.MAILJET_WAVE_LIMIT || String(hourly), 10);
  return Number.isFinite(n) && n > 0 ? n : hourly;
}
