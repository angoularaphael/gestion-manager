...
async function readBotJson(res) {
  ...
  if (isHtmlResponse(trimmed)) {
    const isCloudflare =
      /trycloudflare\.com|cloudflare/i.test(requireBotUrl()) ||
      /cloudflare|no-js ie6 oldie/i.test(trimmed);
    if (isCloudflare) {
      throw new Error(
        'Tunnel Cloudflare expiré ou bot Bothosting arrêté (page HTML au lieu de JSON). ' +
          'Redémarrez le bot, copiez la nouvelle URL https://….trycloudflare.com dans Vercel ' +
          '(WHATSAPP_BOT_URL + NEXT_PUBLIC_WHATSAPP_BOT_URL), puis redéployez.'
      );
    }
    throw new Error(
...
export async function botFetch(path, { method = 'GET', body } = {}) {
  ...
  try {
    ...
    if (msg.includes('fetch failed') || msg.includes('abort') || msg.includes('timeout')) {
      let portHint = '';
      try {
        const { port, protocol } = new URL(requireBotUrl());
        if (protocol === 'http:' && port && !['80', '443', ''].includes(port)) {
          portHint =
            ` Vercel n’accepte que les ports 80/443 en sortie (votre bot est sur :${port}). ` +
            'Utilisez un tunnel HTTPS (Cloudflare) et mettez WHATSAPP_BOT_URL=https://…';
        }
      } catch {
        /* ignore */
      }
      throw new Error(
        'Impossible de joindre le bot Bothosting depuis Vercel.' +
          portHint +
          ' Vérifiez que le bot tourne, que WHATSAPP_BOT_URL est correct, puis redéployez Vercel.'
      );
    }
    throw e;
  }
...
export async function parseClientJson(res) {
  ...
  try {
    ...
    if (isHtmlResponse(raw)) {
      throw new Error(
        'Tunnel Cloudflare expiré ou bot indisponible. Redémarrez Bothosting, mettez à jour WHATSAPP_BOT_URL sur Vercel, puis redéployez.'
      );
    }
