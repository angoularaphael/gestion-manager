...
export async function probeBotAt(baseUrl) {
  ...
  try {
    let data = null;
    try {
      data = await fetchBotPath('/api/health', 8000);
      if (data.connected) {
        return statusFromBotPayload({ ...data, qr: null, qrError: null });
      }
    } catch (e) {
      lastError = e;
    }
    data = await fetchBotPath('/api/status', 18000);
    return statusFromBotPayload(data);
  } catch (e) {
    const msg = String(e.message || e);
    lastError = isTransientNetworkError(msg)
    ...
    if (isTransientNetworkError(msg)) {
      try {
        ...
        return {
          ...statusFromBotPayload({ ...health, qr: null, qrError: null }),
          error: null,
        };
      } catch {
