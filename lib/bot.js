  ...
  if (!url) {
    throw new Error(
      'Bot non configuré : ajoutez NEXT_PUBLIC_WHATSAPP_BOT_URL ou WHATSAPP_BOT_URL_* sur Vercel ' +
        '(ex. http://us2.bot-hosting.net:21334 — voir compta-boxing) puis redéployez.'
    );
  }
  if (url.startsWith('https://')) {
    throw new Error(
      'L\'URL du bot doit être en HTTP (ex. http://us2.bot-hosting.net:21334). ' +
  ...
  if (isHtmlResponse(trimmed)) {
    throw new Error(
      'Bot inaccessible (page HTML reçue). Vérifiez que Bothosting tourne et que ' +
        'NEXT_PUBLIC_WHATSAPP_BOT_URL=http://us2.bot-hosting.net:21334 sur Vercel.'
    );
  }
...
function statusFromBotPayload(data) {
  return {
    configured: true,
    reachable: true,
...
export async function probeBotAt(baseUrl) {
  ...
  async function fetchBotPath(path, timeoutMs) {
  ...
  const statusTimeout = 25000;

  try {
    const data = await fetchBotPath('/api/status', statusTimeout);
    return statusFromBotPayload(data);
  } catch (statusErr) {
    const statusMsg = String(statusErr.message || statusErr);
    try {
      const health = await fetchBotPath('/api/health', 8000);
      if (health.connected) {
        return statusFromBotPayload({ ...health, qr: null, qrError: null });
      }
      return {
        configured: true,
        reachable: true,
        ...
        error: isTransientNetworkError(statusMsg)
          ? 'Le bot répond lentement depuis Vercel — réessayez Actualiser.'
          : statusMsg,
      };
    } catch {
      const msg = isTransientNetworkError(statusMsg)
...
function unreachableMessage(cause) {
  const base =
    ...
    '(http://us2.bot-hosting.net:21334), que Bothosting tourne, puis redéployez.';
...
/** Connexion QR : pas de secret requis (endpoints publics sur boxing-center-bot). */
function requiresBotSecret(path, method) {
  if (method === 'GET') return false;
  if (path === '/api/start' || path === '/api/stop' || path === '/api/logout') return false;
  return true;
}
...
export async function botFetch(path, { method = 'GET', body, timeoutMs: timeoutOverride, baseUrl } = {}, attempt = 0) {
  ...
  if (requiresBotSecret(path, method) && !SECRET()) {
    throw new Error(
