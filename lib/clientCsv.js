import { parseCampaignImportFile } from './balmaImport';
...
export function parseClientImportFile(text, filename = '', { wave = 'all' } = {}) {
  const campaign = parseCampaignImportFile(text, filename, { wave });
  if (campaign?.rows?.length) return campaign.rows;
