...
import { testContactFallback } from './testSendTargets';
...
export async function fetchClientStatsFromDb() {
  ...
  const [totalRes, emailRes, phoneRes] = await Promise.all([
    ...
    sb.from(TABLE).select('id', { count: 'exact', head: true }).not('email', 'is', null).neq('email', ''),
    sb.from(TABLE).select('id', { count: 'exact', head: true }).not('telephone', 'is', null).neq('telephone', ''),
  ]);
...
const CLIENT_LIST_COLUMNS =
  'id, prenom, nom, email, telephone, salle, source, created_at, updated_at';

export async function fetchClientsFromDb({ search = '', source = '', salle = '' } = {}) {
  const sb = getSupabase();
  const makeQuery = () =>
    applyClientListFilters(
      sb.from(TABLE).select(CLIENT_LIST_COLUMNS).order('created_at', { ascending: false }),
      { search, source, salle }
    );
...
export async function fetchClientsByIds(ids) {
  ...
  const sb = getSupabase();
  const { data, error } = await sb.from(TABLE).select(CLIENT_LIST_COLUMNS).in('id', ids);
  if (error) throw new Error(error.message);
  return data || [];
}
...
export async function createClientInDb(body) {
  ...
  const row = touchRow({
    ...
    salle: fields.salle,
    source: ['csv', 'xls', 'manual', 'chatbot', 'boxplus'].includes(body.source) ? body.source : 'manual',
  });
...
export async function importClientFieldsList(rows) {
  ...
  for (const fields of uniqueRows) {
    ...
    if (id) {
      stats.duplicates++;
      if (fields.telephone) {
        toPatch.push({ id, telephone: fields.telephone, ...touchRow({}) });
      }
      continue;
    }
    toInsert.push(row);
  ...
  for (let i = 0; i < toPatch.length; i += INSERT_CHUNK) {
    const chunk = toPatch.slice(i, i + INSERT_CHUNK);
    await Promise.all(
      chunk.map(async ({ id, telephone, updated_at }) => {
        ...
        if (existing?.telephone) return;
        const { error } = await sb.from(TABLE).update({ telephone, updated_at }).eq('id', id);
        if (error) throw new Error(error.message);
        stats.updated++;
      })
    );
