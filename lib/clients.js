...
function phoneLookupVariants(phone) {
  ...
  if (!key) return [];
  return [...new Set([key, `0${key}`, `33${key}`, `+33${key}`])];
}
...
export async function importClientFieldsList(rows) {
  ...
  if (phones.length) {
    ...
    if (error) throw new Error(error.message);
    for (const client of data || []) {
      const key = cleanPhone(client.telephone);
  ...
  for (const fields of uniqueRows) {
    const id =
      ...
      (fields.telephone && phoneToId.get(cleanPhone(fields.telephone))) ||
      null;
    ...
    if (error) {
      for (const row of chunk) {
        try {
          ...
          if (one.telephone) phoneToId.set(one.telephone, one.id);
        } catch (err) {
      ...
      for (const client of data) {
        if (client.email) emailToId.set(normalizeEmail(client.email), client.id);
        if (client.telephone) phoneToId.set(client.telephone, client.id);
      }
    }
  }
