...
import { sendEmailViaResend } from './resendSend';
import { sendEmailViaSes } from './sesSend';
...
export async function deliverEmail({
  ...
  if (provider === 'ses') {
    try {
      ...
      if (!allowBotFallback) {
        throw new Error(`Amazon SES : ${e.message || e}`);
      }
      return await botFetch('/api/send-email', { method: 'POST', body: botPayload });
    }
  }
  ...
  if (provider === 'mailjet') {
    try {
      return await sendEmailViaMailjet({
        ...
        mailjetAccount,
        customHeaders: isMarketing ? customHeaders : undefined,
        inlinedAttachments: isMarketing ? inlinedAttachments : undefined,
      });
    } catch (e) {
      if (!allowBotFallback) {
