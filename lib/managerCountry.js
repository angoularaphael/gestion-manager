...
export function hasUsableContact(manager) {
  return Boolean(manager?.email || manager?.telephone);
}

export function filterManagers(
  managers,
  { search = '', type = '', country = '', countries = [], excludeNoContact = false } = {}
) {
  ...
  return managers.filter((m) => {
    if (excludeNoContact && !hasUsableContact(m)) return false;
