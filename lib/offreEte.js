...
import { resetCampaignWhatsAppTracking } from './campaignWhatsApp';
...
async function fetchOffreEteWhatsAppStats() {
  try {
    ...
    return {
      whatsappSent: display.whatsappSentOverride ?? whatsappSent,
      whatsappRead: display.whatsappReadOverride ?? whatsappRead,
      whatsappOpenRate:
        (display.whatsappSentOverride ?? whatsappSent) > 0
          ? Math.round(
              ((display.whatsappReadOverride ?? whatsappRead) /
                (display.whatsappSentOverride ?? whatsappSent)) *
                100
            )
          : null,
    ...
    const display = getOffreEteWhatsAppDisplayStats();
    if (display.whatsappSentOverride != null || display.whatsappReadOverride != null) {
      const sent = display.whatsappSentOverride ?? 0;
      const read = display.whatsappReadOverride ?? 0;
      return {
        whatsappSent: sent,
        whatsappRead: read,
        whatsappOpenRate: sent > 0 ? Math.round((read / sent) * 100) : null,
        recentWhatsAppReads: [],
      };
    }
    return {
