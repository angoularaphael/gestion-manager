  ...
  if (sendEmail) {
    tasks.push((async () => {
      ...
      try {
        ...
        delivery.errors.push({ channel: 'email', error: e.message || 'Modèle email invalide' });
        return;
      }
      try {
        delivery.email = await deliverEmail({
          to: email,
          subject,
          text,
          html,
          recipientName: name || '',
          mailjetAccount: 'credentials',
        });
      } catch (e) {
        delivery.errors.push({ channel: 'email', error: e.message || String(e) });
      }
    })());
  }
